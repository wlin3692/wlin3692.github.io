class Play extends Scene {
    ...
    event() {
        ...
        // 移动端触控按钮复用玩家动作，按住时持续移动。
        const actions = { up: 'up', down: 'down', left: 'left', right: 'right' };
        $s('.touch-controls [data-control]').forEach(button => {
            const control = button.dataset.control;
            let timer = null;
            const stop = () => { if (timer) { clearInterval(timer); timer = null; } };
            const start = (event) => {
                event.preventDefault();
                if (control === 'fire') { this.player.fire(); res.replay('shoot'); return; }
                const move = () => this.player && this.player[actions[control]]();
                move();
                timer = setInterval(move, 45);
            };
