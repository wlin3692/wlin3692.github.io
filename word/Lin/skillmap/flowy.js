var flowy = function(canvas, grab, release, snapping, rearrange, spacing_x, spacing_y) {
    ...
    flowy.load = function() {
        ...
        flowy.import = function(output) {
            ...
            if (blocks.length > 1 && !output.preserveLayout) {
                rearrangeMe();
                checkOffset();
            }
        }
        ...
        function rearrangeMe() {
            var root = blocks.find(function(block) { return block.parent == -1; });
            if (!root) return;

            function childrenOf(id) {
                return blocks.filter(function(block) { return block.parent == id; });
            }

            function measure(block) {
                var children = childrenOf(block.id);
                if (!children.length) {
                    block.childwidth = 0;
                    return block.width;
                }
                ...
                children.forEach(function(child, index) {
                    total += Math.max(child.width, measure(child));
                    if (index < children.length - 1) total += paddingx;
                });
                block.childwidth = total;
                return Math.max(block.width, total);
            }

            function place(block) {
                var children = childrenOf(block.id);
                if (!children.length) return;
                var total = children.reduce(function(width, child, index) {
                    return width + Math.max(child.width, child.childwidth) + (index ? paddingx : 0);
                }, 0);
                var cursor = block.x - total / 2;

                children.forEach(function(child) {
                    var span = Math.max(child.width, child.childwidth);
                    child.x = cursor + span / 2;
                    child.y = block.y + block.height / 2 + paddingy + child.height / 2;
                    cursor += span + paddingx;

                    var childElement = document.querySelector(".blockid[value='" + child.id + "']").parentNode;
                    childElement.style.left = child.x - child.width / 2 + "px";
                    childElement.style.top = child.y - child.height / 2 + "px";

                    var arrowElement = document.querySelector('.arrowid[value="' + child.id + '"]').parentNode;
                    arrowElement.style.top = block.y + block.height / 2 + "px";
                    var arrowx = child.x - block.x + 20;
                    updateArrow(child, arrowx, paddingy, child);
                    place(child);
                });
            }
