...
renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2)); // 防止高倍屏炸性能
document.body.appendChild(renderer.domElement);

camera.position.z = 1;

// 触摸适配：TrackballControls 需要加 { passive: false }
const controls = new THREE.TrackballControls(camera, renderer.domElement);
controls.noPan = true;
controls.maxDistance = 3;
controls.minDistance = 0.7;
controls.rotateSpeed = 0.8; // 手机触控更灵敏
...
function init() {
  const count = window.innerWidth < 768 ? 5000 : 20000; // 手机降负载
  for (let i = 0; i < count; i++) spikes.push(new Grass());
}
...
function render(a) {
  positions.length = 0; // 复用数组，减少 GC
  spikes.forEach(g => {
    g.update(a);
    positions.push(g.one.x, g.one.y, g.one.z, g.two.x, g.two.y, g.two.z);
  });
